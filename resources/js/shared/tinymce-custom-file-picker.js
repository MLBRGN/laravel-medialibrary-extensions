...
window.mleFilePicker = (callback, value, meta) => {
    ...
    if (meta.filetype === 'image') {
        ...
        const baseId = textarea.getAttribute('data-base-id') || textarea.id || textarea.name || 'media-manager';
        ...
        function handleMessage(event) {
            ...
            try {
                ...
                if (form && pickedInstanceId) {
                    // Search for existing input with same value to avoid duplicates
                    const existing = Array.from(form.querySelectorAll('input[name="mle_instance_ids[]"]'))
                        .find(el => el.value === pickedInstanceId);

                    if (!existing) {
                        const i = document.createElement('input');
                        i.type = 'hidden';
                        i.name = 'mle_instance_ids[]';
                        i.value = pickedInstanceId;
                        form.appendChild(i);
                    }
                }
            } catch (e) {
                console.warn('Could not persist instance_id to form', e);
            }

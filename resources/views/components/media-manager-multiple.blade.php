                                ...
                                <div class="">
                                        @if($setAsFirstEnabled)
                                            @if($medium->order_column === $media->min('order_column'))
                                                <button class="" title="{{ __('media-library-extensions::messages.set-as-main') }}" disabled>
                                                    <x-mle-icon name="{{ config('media-library-extensions.icons.set-as-main') }}" title="{{ __('media-library-extensions::messages.set-as-main') }}" class="w-4 h-4" />
                                                </button>
                                            @else
                                            <form class="media-manager-menu-form" action="{{ route(mle_prefix_route('set-as-first')) }}" method="post">
                                                ...
                                                <input type="hidden" name="model_type" value="{{ get_class($model) }}">
                                                <input type="hidden" name="model_id" value="{{ $model->id }}">
                                                <button type="submit" class="" title="{{ __('media-library-extensions::messages.setup-as-main') }}">
                                                    <x-mle-icon name="{{ config('media-library-extensions.icons.setup-as-main') }}" title="{{ __('media-library-extensions::messages.setup-as-main') }}" class="w-4 h-4" />
                                                </button>
                                            </form>
                                            @endif
                                        @endif
                                ...
                                <div class="media-manager-preview-image-menu-end d-flex align-items-center gap-1">
                                    @if($destroyEnabled)
                                        <form class="@mediaClass('media-manager-multiple-preview-medium-form')" action="{{ route(mle_prefix_route('medium-destroy'), $medium->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="" title="{{ __('media-library-extensions::messages.delete_medium') }}">
                                                <x-mle-icon name="{{ config('media-library-extensions.icons.delete') }}" :title="__('media-library-extensions::messages.delete_medium')" class="w-4 h-4" />
                                            </button>
                                        </form>
                                    @endif
                                </div>

...
//        dd($collection);
        if ($modelOrClassName instanceof HasMedia) {
            $model = $this->modelOrClassName;
            $this->medium = $model->getFirstMedia($collection);
            $this->youtubeId = $this->medium->getCustomProperty('youtube-id');
        } elseif (is_string($modelOrClassName)) {
            $youtubeVideos = TemporaryUpload::forCurrentSession($collection);
            if ($youtubeVideos->count() > 0) {
                $this->medium = $youtubeVideos->first();
                $this->youtubeId = $this->medium->getCustomProperty('youtube-id');
            }
//            $this->medium = TemporaryUpload::forCurrentSession($collection)->first();
        } else {
    ...
    {
        return view('media-library-extensions::components.youtube-video');
    }
}

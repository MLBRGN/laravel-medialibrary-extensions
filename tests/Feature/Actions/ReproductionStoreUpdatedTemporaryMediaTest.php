...
beforeEach(function () {
    ...
    config(['medialibrary-extensions.data_sources.media_demo' => [
...
it('correctly replaces a temporary upload on a custom data source', function () {
    ...
    if (! $check) {
        ...
        $checkDefault = TemporaryUpload::on('testbench')->find($oldId);
        if ($checkDefault) {
    ...
    expect(TemporaryUpload::on('testbench')->find($newId))->toBeNull();
